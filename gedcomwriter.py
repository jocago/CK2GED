...
class GedcomWriter(object):
    ...
    def generate_gedcom_families(self):
        print('### Generating GEDCOM family information...',)
        ...
        for c in self.character_map:
            ...
            if settings.real_fathers and character.real_father in self.character_map:
            ...
            if father_id in self.character_map and self.character_map[father_id].mark:
                generate_parents = True
            elif mother_id in self.character_map and self.character_map[mother_id].mark:
                generate_parents = True
            else:
            ...
            if len(character.spouse) > 0 and not settings.cull_childless_spouses:
    ...
    def write_gedcom(self, filename):
        ...
        with open(filename, 'w') as file:
            ...
            for g in self.gedcom_map:
                ...
                line += '\n1 SEX ' + gender_string
                line += '\n1 OCCU ' + character.get_primary_title(self.title_map)
                ...
                elif settings.real_fathers and character.real_father in self.character_map:
                    line += '\n1 NOTE Father unknown'

                elif not settings.real_fathers \
                        and character.real_father in self.character_map:
                    line += '\n1 NOTE Real father is '
                    line += self.character_map[character.real_father].birth_name + ' '
                    line += self.character_map[character.real_father].dynasty_name + ' ('
                    line += str(character.real_father) + ')'

                if type(character.birthday) is Date:
                    line += '\n1 BIRT\n2 DATE ' + character.birthday.gedcom_string()

                if type(character.deathday) is Date:
                    line += '\n1 DEAT\n2 DATE ' + character.deathday.gedcom_string()
